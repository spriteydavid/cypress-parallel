...
function distributeTestsByWeight(testSuitePaths) {
  ...
  for (const [key] of map.entries()) {
    threads[index % settings.threadCount].list.push(key);
    index++;
  }

  // Run slowest group first
  threads.sort((a, b) => {
    const aWeight = a.list.reduce((acc, cur) => acc + map.get(cur), 0);
    const bWeight = b.list.reduce((acc, cur) => acc + map.get(cur), 0);
