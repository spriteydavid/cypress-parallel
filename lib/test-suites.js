...
function distributeTestsByWeight(testSuitePaths) {
  ...
  for (const [key, value] of map.entries()) {
    let lowestWeightThreadIndex = 0;
    let lowestWeight = Number.MAX_VALUE;

    for (let i = 0; i < threads.length; i++) {
      const currentThreadWeight = threads[i].reduce((acc, cur) => acc + map.get(cur), 0);
      if (currentThreadWeight < lowestWeight) {
        lowestWeight = currentThreadWeight;
        lowestWeightThreadIndex = i;
      }
    }

    threads[lowestWeightThreadIndex].push(key);
  }
